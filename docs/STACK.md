...
├── di/ServiceLocator.kt             OkHttp/Retrofit/Room/Repository/Notifier 싱글톤
├── data/
...
│   │   ├── NetworkModule.kt         OkHttp(User-Agent, logging) + Retrofit + Json
...
├── worker/MaCrossoverWorker.kt      CoroutineWorker, 주기 갱신·알림
└── ui/
    ├── theme/                       Color.kt, Theme.kt
    ├── nav/
    │   ├── Destinations.kt          BottomBar 탭 enum (Search/Watchlist/Dashboard)
...
- **일봉 차트**: `GET https://query1.finance.yahoo.com/v8/finance/chart/{symbol}?range=3mo&interval=1d`
  - `range`: `1mo`, `3mo`, `6mo`, `1y` 등
  - 응답에서 `chart.result[0].timestamp[]`와 `indicators.quote[0].close[]` 파싱
- **공통 헤더**: `User-Agent: Mozilla/5.0 ...` (OkHttp Interceptor에서 주입)
- **심볼 예시**: 삼성전자 `005930.KS`, SK하이닉스 `000660.KS`, Apple `AAPL`, Tesla `TSLA`
- 4xx/네트워크 실패는 Repository가 `Result` 래핑으로 삼킴 → Worker는 다음 주기 재시도.

**주의**: 비공식 API라 스키마가 언제든 변할 수 있고 일부 통신사/국가에서 차단 가능. 장기 안정성이 필요하면 한국투자증권 KIS OpenAPI 등 공식 소스로 교체.
...
- 장 외 시간에도 계속 돌지만 종가가 변하지 않으니 DB upsert는 동일 값, 알림 발송 0건. 장 시각 게이팅은 휴장일·서머타임·반장 때문에 오히려 복잡도 증가라 도입 안 함.
